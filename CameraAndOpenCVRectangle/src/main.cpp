...
void processing()
{
	...
	while (1)
	{
		...
		xDeltaFromCenter = minIndex.x - (xAnchor + ROIWidth / 2);
		yDeltaFromCenter = minIndex.y - (yAnchor + ROIHeight / 2);
		...
		line(localFrame, Point(xAnchor + ROIWidth / 2, yAnchor + ROIHeight / 2 + 10), Point(xAnchor + ROIWidth / 2, yAnchor + ROIHeight / 2 - 10), Scalar(0, 0, 255), 3);
		line(localFrame, Point(xAnchor + ROIWidth / 2 + 10, yAnchor + ROIHeight / 2), Point(xAnchor + ROIWidth / 2 - 10, yAnchor + ROIHeight / 2), Scalar(0, 0, 255), 3);
...
void userInterface()
{
	...
	//namedWindow("Resized (full image)");
	//namedWindow("zoom-in (original size)");
	...
	while (1)
	{
		...
		if (sharedCirclesMutex.try_lock() == true)
		{
			...
			imshow("zoom-in (original size)", localFrame(Rect(imageWidth * 3 / 8, imageHeight * 3 / 8, imageWidth / 4, imageHeight / 4)));

			// Resize:
			resize(localFrame, localFrame, Size(640, 480));

			imshow("Resized(full image)", localFrame);
		}
...
//	printf("	t for toggling tracking-enable state \n");
	...
	std::cout << "width = " << pCam->Width.GetValue() << std::endl;
	std::cout << "Height = " << pCam->Height.GetValue() << std::endl;
